...
Intent는 다른 앱 구성요소에 작업을 요청하는 데 사용할 수 있는 메시지 객체(안드로이드 시스템에게 보내는 표준화된 편지)이고, 인텐트는 여러 방식으로 구성요소 간 통신을 지원한다.   
...
* Nested Intent 대신 PendingIntent를 사용
    ...
    * 알림과 같은 기능을 구현할때 PendingIntent를 사용한다.
        * 알림은 시스템이 그리는 UI라 앱이 직접 클릭 이벤트를 받을 수 없다. -> 시스템이 대신 Intent를 실행해야함(시스템에게 해당 Intent를 실행할 권한을 주는것)
        * fcm 푸시는 앱이 꺼져 있어도 와야 하고, 나중에 탭했을 때도 동작해야함 -> 나중 실팽용 Intent가 필요함(시스템이 내부적으로 Intent와 호출 정보를 저장해둠)
        * 실행되는 동작은 우리 앱의 권한으로 수행되어야 하고, 마음대로 수행되면 안됨 -> 보안적으로 해팅된 토큰이 필요함
